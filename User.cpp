...
double akt_regler = 0.0;
int wechsel = 0;
double driver_limit = -1;
double driver_safety_dist = -1;

sensor_data sens_data;

double PID_output, p_anteil, i_anteil, d_anteil;
double dist;
...
//double change_lane_parameter;
//int counter_tolerance[6] = { 0 };



Umgebungsueberwachung umgebung;
...
{
    int i;
    //DDefInt(NULL, "MyCounter", "", &counter, DVA_None);
    for (i=0; i<N_USEROUTPUT; i++) {
	char sbuf[32];
	sprintf (sbuf, "UserOut_%02d", i);
	DDefDouble (NULL, sbuf, "", &User.Out[i], DVA_IO_Out);
    DDefDouble(NULL, "PID_output", "" , &PID_output, DVA_None);
    DDefDouble(NULL, "P_Anteil", "", &p_anteil, DVA_None);
    DDefDouble(NULL, "I_Anteil", "", &i_anteil, DVA_None);
    DDefDouble(NULL, "D_Anteil", "", &d_anteil, DVA_None);
    DDefDouble(NULL, "speed_limit", "", &speed_limit, DVA_None);
    DDefDouble(NULL, "max_turn_speed", "m/s", &max_turn_speed, DVA_None);
    DDefDouble(NULL, "dist_to_object", "", &dist, DVA_None);
    DDefDouble(NULL, "aktiver_regler", "", &akt_regler, DVA_None);
    DDefInt(NULL, "wechsel", "", &wechsel, DVA_VC);
    DDefDouble(NULL, "driver_limit", "", &driver_limit, DVA_VC);
    DDefDouble(NULL, "driver_safety_dist", "", &driver_safety_dist, DVA_VC);
    //DDefInt(NULL, "counter0", "", &counter_tolerance[0], DVA_None);
    //DDefInt(NULL, "counter1", "", &counter_tolerance[1], DVA_None);
...
User_VehicleControl_Calc(double dt)
{
    ...
    //Log("dt: %lf \n", dt);
    //Log("wechsel in user: %i \n", wechsel);
    umgebung.Moduswahl(sim_time, wechsel, driver_limit, driver_safety_dist);  //Moduswahl Funktion aufrufen. Manuelle gesteuerte Wert von dem Parameter wechsel wird hier uebergeben. sonst ist wechsel gleich

    wechsel = umgebung.get_wechsel();   //Parameter wechsel wird aktualisiert, damit in User.cpp und Umgebungsueberwachung.cpp das Parameter uebereinstimmen.
    sens_data = umgebung.get_sensor_data();
    speed_limit = sens_data.limit;
    akt_regler = double(sens_data.aktiver_regler);
    max_turn_speed = sens_data.turn_max_speed;
    dist = ObjByLane[0].Lane[1][0].ObjFront[0].sMin;
    PID_output = sens_data.regler_values.sum_out;
    p_anteil = sens_data.regler_values.p_out;
    i_anteil = sens_data.regler_values.i_out;
    d_anteil = sens_data.regler_values.d_out;
